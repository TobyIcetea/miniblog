...
// WithMetadata 设置元数据
func (err *ErrorX) WithMetadata(metadata map[string]string) *ErrorX {
	err.Metadata = metadata
	return err
}

// KV 使用 key-value对 设置元数据
func (err *ErrorX) KV(kvs ...string) *ErrorX {
	if err.Metadata == nil {
		err.Metadata = make(map[string]string) // 初始化元数据映射
	}

	for i := 0; i < len(kvs) && i+1 < len(kvs); i += 2 {
...
func (err *ErrorX) GRPCStatus() *status.Status {
	detail := errdetails.ErrorInfo{Reason: err.Reason, Metadata: err.Metadata}
	s, _ := status.New(httpstatus.ToGRPCCode(err.Code), err.Message).WithDetails(&detail)
	return s
}
...
// FromError 尝试将一个通用的 error 转化为自定义的 *ErrorX 类型
