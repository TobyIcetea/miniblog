...
func TestErrorX_FromPlainError(t *testing.T) {
	// 创建一个普通的 Go 错误
	...
	// 检查转换后的 ErrorX
	assert.Equal(t, errorsx.ErrInternal.Code, errx.Code)
	assert.Equal(t, errorsx.ErrInternal.Reason, errx.Reason)
	assert.Equal(t, "Something went wrong", errx.Message)
}
