...
var (
	...
	password   = pflag.StringP("password", "p", "miniblog1234", "Password to connect to the database.")
...
func processComponent(component string, dbInstance *gorm.DB) {
	...
	if !ok {
		log.Printf("Component %s not found", component)
		return
	}
...
func createGenerator(packagePath string) *gen.Generator {
	return gen.NewGenerator(gen.Config{
		...
		FieldSignable:     false, // 禁用有符号属性以提高兼容性。
