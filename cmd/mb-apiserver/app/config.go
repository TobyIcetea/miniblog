...
func onInitialize() {
	if configFile != "" {
		...
		// 配置文件名称（没有文件扩展名）
		viper.SetConfigName(strings.TrimSuffix(defaultConfigName, filepath.Ext(defaultConfigName)))
	}

	// 读取环境变量并设置前缀
	setupEnvironmentVariables()

	// 读取配置文件。如果制定了配置文件名，则使用指定的配置文件，否则在注册的搜索路径中搜索
...
// setupEnvironmentVariables 设置环境变量规则
