...
// 该接口包含了项目中支持的日志记录方法，提供对不同级别日志级别的支持
type Logger interface {
	...
	// Warnw 用于记录警告级别的日志，表示可能存在问题但不影响系统正常与性能
	Warnw(msg string, kvs ...any)

	// Errorw 用记录错误级别的日志，表示系统运行中出现的错误，需要开发人员介入处理
	Errorw(msg string, kvs ...any)

	// Panicw 用于记录严重错误级别的日志，表示系统无法继续鱼腥，记录日之后会触发 panic
	Panicw(msg string, kvs ...any)

	// Fatalw 用记录致命错误级别的日志，表示系统无法继续运行，记录日志后会直接退出程序
...
// Init 初始化全局的日志对象，初始化包级变量 std
func Init(opts *Options) {
	// 因为会给全局变量 std 赋值，所以这里对 std 变量枷锁，防止出现并发问题
	mu.Lock()
	defer mu.Unlock()
...
// 如果 Options 参数为空，这会使用默认的 Options 配置
func New(opts *Options) *zapLogger {
	...
	// 自定义 MessageKey 为 message, message 语义明明确
