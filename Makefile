...
VERSION_PACKAGE=github.com/TobyIcetea/miniblog/pkg/version
## 定义 VERSION 语义化版本
...
add-copyright: # 添加版权头信息
	@addlicense -v -f $(PROJ_ROOT_DIR)/scripts/boilerplate.txt -ignore third_party/ -ignore vendor/ -ignore $(OUTPUT_DIR)/ $(PROJ_ROOT_DIR)
...
ca: # 生成 CA 文件
	...
	@openssl req -new -key $(OUTPUT_DIR)/cert/ca.key -sha256 -days 3650 -out $(OUTPUT_DIR)/cert/ca.csr \
		-subj "/C=CN/ST=Shanxi/L=Xian/O=onexstack/OU=it/CN=127.0.0.1/emailAddress=x2406862525@163.com"
	...
	@# 5. 使用服务端私钥生成服务端证书签名请求（Server CSR）
	@openssl req -new -key $(OUTPUT_DIR)/cert/server.key -out $(OUTPUT_DIR)/cert/server.csr \
		-subj "/C=CN/ST=Shanxi/L=Xian/O=serverdevops/OU=serverit/CN=localhost/emailAddress=x2406862525@163.com" \
