...
func main() {
	...
	// 继续向错误中添加元数据，这次使用了 KV 方法，它是一种更加简洁的方法，用 key-value 的模式逐一设置元数据
	...
	// Node: 更新字段并不会影响 Code、Reason 和 Metadata，它只是说明错误的上下文发生了变化
...
func doSomething() error {
	...
	// 重点是：虽然错误的 Message 不同，但是错误的 Code 和 Reason 是一致的，这方便使用 Is 方法进行类型判断而不受具体内容影响
